...
# Pragma Agency Agents for Windows

把上游 Agency Agents 的 Markdown Skills 作为 Pragma Desktop Bundle 的内置能力使用。优先执行本 Skill 的确定性脚本；脚本不可用时，按下述顺序完成同样的检查与回滚边界。
...
5. 导入后必须核对：270 个 Agency 专家资源、270 个内置 Skill 能力、Bundle 安装记录为 `ready`、目标 Team 的 `isRefPending` 为 `false`，并确认 Mission 创建前置依赖检查通过。
6. 需要启动时调用 `scripts/start_pragma.ps1`；需要单独创建快捷方式时调用 `scripts/create_pragma_shortcut.ps1`。快捷方式指向启动脚本而不是临时工作目录。

## 真实应用场景

- **首次安装**：用户说“在这台 Windows 电脑上装好 Pragma 和 270 个 Agency Agents 专家，装完直接能用”。
...
> “安装并配置 Pragma Agency Agents，默认导入 270 个专家，创建桌面快捷方式，最后验证能不能新建 Mission。”
